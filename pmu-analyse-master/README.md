...
2. Sur [share.streamlit.io](https://share.streamlit.io), créer une nouvelle app en pointant sur ce repo, la branche `main`, et le fichier principal `app.py`.
3. Aucun secret n'est requis pour l'instant (l'API PMU utilisée est publique, sans clé). Si des identifiants sont ajoutés plus tard, les déclarer dans la section "Secrets" de l'app (jamais dans le repo).
4. La base SQLite (`database/db/pmu_data.db`) est committée avec des données de démo (1–9 janvier 2024) pour que l'app affiche quelque chose dès le premier chargement. Le stockage de Streamlit Cloud est éphémère : toute donnée scrapée en ligne sera perdue au redémarrage/redéploiement de l'app, sauf à la re-committer manuellement ou à migrer vers une base externe persistante.

## Exécuter le scraping en CLI (sans interface)
