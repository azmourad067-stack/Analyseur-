...
def programme_choices(programme: Any) -> list[dict[str, Any]]:
    """Best-effort extraction of reunion/course selectors from PMU JSON."""
    candidates = _find_list_of_dicts(programme, ("numReunion", "numCourse", "numOrdre", "numOfficiel"))
    out = []
    seen = set()
    for item in candidates:
        reunion = item.get("numReunion") or item.get("numOfficiel") or item.get("numReunionProgramme")
        course = item.get("numCourse") or item.get("numOrdre")
        ...
        if key in seen:
            continue
        seen.add(key)
        label = item.get("libelle") or item.get("nom") or item.get("nomCourse") or f"R{r}/C{c}"
        out.append({"reunion": r, "course": c, "label": label})
    return sorted(out, key=lambda x: (x["reunion"], x["course"]))
