...
## Import historique intégré

Le dépôt inclut maintenant `scripts/import_history.py`.
...
La source historique utilisée est l'API communautaire `open-pmu-api.vercel.app`. Le script importe jour par jour, normalise les courses et les partants, renseigne les arrivées, puis effectue des upserts idempotents dans Supabase.

### CSV

```bash
python scripts/import_history.py csv data/template_historique_v3.csv
```
...
Variables requises pour les scripts d'administration :
...
Dans Streamlit Cloud, utiliser `SUPABASE_URL` + `SUPABASE_KEY` pour la lecture de l'application. Ne jamais exposer la service key dans le dépôt GitHub.

Avant le premier entraînement, exécuter `database/migration_temporal_history.sql` dans Supabase afin d'installer la fonction temporelle utilisée par le modèle.
